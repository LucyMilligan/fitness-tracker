...
class TestCreateActivity:
    ...
    def test_create_activity_raises_422_for_incorrect_date_field(self, client: TestClient):
        activity_test = {
            "user_id": 1,
    ...
    def test_create_activity_raises_422_for_multiple_incorrect_fields(self, client: TestClient):
        activity_test = {
            "user_id": 1,
            "date": "25 March 25", #incorrect format
...
class TestUpdateActivity:
    def test_endpoint_returns_200_status_code_on_success_non_validated_fields(self, client: TestClient, session: Session, activity_test_1):
        activity_1 = Activity(**activity_test_1)
        session.add(activity_1)
        session.commit()
    ...
    def test_endpoint_returns_200_status_code_on_success_validated_fields(self, client: TestClient, session: Session, activity_test_1):
        activity_1 = Activity(**activity_test_1)
        session.add(activity_1)
        session.commit()
    ...
    def test_update_activity_raises_422_for_incorrect_fields(self, session: Session, client: TestClient, activity_test_1):
        activity_1 = Activity(**activity_test_1)
        session.add(activity_1)
        session.commit()
